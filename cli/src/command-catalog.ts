...
export const COMMANDS: CommandCatalogEntry[] = [
  ...
  { name: 'get-selection', description: 'Serialize the current selection [--depth 1]' },
  {
    ...
    description:
      ...
      + 'mainComponentError or childrenError, a collapsed.readErrors above 0, or a minimal '
      + '{id, readError} record whose own identity read refused (still counted in emitted). '
      ...
      + 'itself, measured 408ms) and budget.devResources {found, attached, readMs, error?} is '
      + 'reported UNCONDITIONALLY, found 0 included — presence means "you asked", so "none here" '
      + 'is never confused with "nobody looked". attached counts the EMITTED records a resource '
      + 'landed on, so attached below found means the rest belong to nodes absent from this reply: '
      + 'descendants the budget or deadline never enqueued, nodes outside --depth, or a record '
      + 'whose identity read refused and which therefore carries no intent), componentKey '
